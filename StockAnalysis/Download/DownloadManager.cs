...
public class DownloadManager
{
    ...
    public virtual async Task<bool> GetHoldings(IEnumerable<HoldingInformation> holdings, HttpClient client)
    {
        try
        {
            foreach (var uri in holdings)
            {
                ...
                if (!await _storage.Store(stream, StoragePath, uri.Name))
                {
                    return false;
                }
            }
